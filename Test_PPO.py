...
def main(game_index: int, mode: str) -> None:
    ...
    if game_index in [1, 2]:
        policy_model = MLPContiModel(env.obs_dim, env.act_dim, hidden_size=(15, 10), hd_activation='Tanh',
    ...
    ppo = PPO(env=env, policy_model=policy_model, evaluate_model=evaluate_model, model_save_dir='MODEL_PARAMS',
              exp_name=exp_name, gamma=gamma, beta=beta, logger=logger, clip_ratio=clip_ratio, policy_lr=0.004,
              evaluate_lr=0.005, is_ou_noise=is_ou_noise)
    if mode == 'TRAIN':
        ppo.train(learning_epochs=learn_epoch, retrain_label=False, update_num=50, save_freq=10,
